...
public class QuizController {
    ...
    @Operation(summary = "퀴즈 채점", description = """
        ...
        """)
    @PostMapping("/grading")
    public ResponseEntity<CommonResponse<QuizGradingResponse>> gradeQuiz(@RequestBody QuizGradingRequest request) {
        QuizGradingResponse result = quizGradingService.grade(request);
        return ResponseEntity.ok(CommonResponse.success(result));
    }

    @Operation(summary = "서바이벌 결과 등록", description = """
    ...
    public ResponseEntity<CommonResponse<Void>> registerSurvivalResult(@PathVariable("studyId") Long studyId, @PathVariable int week, @RequestBody SurvivalResultRequest request) {
        survivalResultService.processSurvivalResult(studyId, week, request);
        return ResponseEntity.ok(CommonResponse.noContent());
    }

    @Operation(summary = "주차별 퀴즈 문제 생성 실제 서비스에서 사용x 테스트 위해서 만든 API", description = """
    ...
    public ResponseEntity<CommonResponse<QuizRegisterResponse>> createQuizProblems(@PathVariable Long studyId, @RequestBody QuizRegisterRequest request) throws IOException {
